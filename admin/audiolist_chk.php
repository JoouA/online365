...
if($grade == 1){
    // 插入一级目录
    $sql = "insert into tb_audiolist(`grade`,`name`,`userName`,`issueDate`) VALUES('$grade','$name','$userName','$date')";
}else{
    // 插入二级目录
    $father = htmlspecialchars(trim($_POST['father']));
    $sql = "insert into tb_audiolist(`grade`,`name`,`father`,`userName`,`issueDate`) VALUES('$grade','$name','$father','$userName','$date')";
}
